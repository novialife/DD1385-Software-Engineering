    ...
    ViewControl (Boardgame gm, int n){  
		...
		for(int i = 0; i < size; i++){
	        for(int j = 0; j < size; j++){
				Square button = new Square(game.getStatus(i, j), i, j);		// Create square btn with text getstatus(i,j) and coordinates i j
				board[i][j] = button;										// Add to board
				button.addActionListener(this);								// Add actionlistener to look for press
				add(button);												// Add to Jframe
			}
		}
		setVisible(true);
	}
